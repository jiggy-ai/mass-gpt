...
from completion import CompletionLimits, Completion
import openai
...
def gpt3_CompletionLimits(min_prompt:int, min_completion:int, max_completion:int):
    """
    CompletionLimits specific to GPT3 models with max_context of 4097 tokens
    """
    return CompletionLimits(max_context    = 4097,
...
def gpt3_inference_func(model                 : str,
                        prompt                : str,                        
                        temperature           : float,
...
def gpt3_CompletionTask(limits,
                        temperature,
    ...
    return CompletionTask(limits         = limits,
                          temperature    = temperature,
                          model          = model,
                          inference_func = gpt3_inference_func)
